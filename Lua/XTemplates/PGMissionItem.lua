...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateTemplate', {
		...
		'OnPress', function (self, gamepad)
			...
			if prop_id == "idMissionSponsor" then
				local sponsor = Presets.MissionSponsorPreset.Default[new_val]
				for _, rule in ipairs(sponsor.incompatible_game_rules or empty_table) do
					g_CurrentMissionParams.idGameRules[rule] = nil
				end
			end
			...
			elseif prop_id == "idCommanderProfile" then
				...
				local commander = Presets.CommanderProfilePreset.Default[new_val]
				for _, rule in ipairs(commander.incompatible_game_rules or empty_table) do
					g_CurrentMissionParams.idGameRules[rule] = nil
				end
