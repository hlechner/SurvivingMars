...
function LayoutConstructionController:PlaceCursors(test)
	...
	if not test then
		realm:SuspendPassEdits("LayoutConstructionController:PlaceCursors")
	else
	...
	if not test then
		realm:ResumePassEdits("LayoutConstructionController:PlaceCursors")
		if self.prefab then
			self.city:AddPrefabs(self.template_name, -1)
			if self.prefab and self.city:GetPrefabs(self.template_name) <= 0 then
				--[[
	...
	for _, entry in ipairs(layout_preset) do
		...
		if entry.template~="life_support_grid" and entry.template~="electricity_grid" then
			...
			for _, resource in ipairs(ConstructionResourceList) do
				local amount = UIColony.construction_cost:GetConstructionCost(template_class, resource, mod_o)
				if amount > 0 then
