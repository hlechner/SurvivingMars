...
function LevelPrefabBuilding_InstantiatePrefab(self, height_op, more_enum_flags_to_clear, per_obj_func)
	...
	if prefab then
		...
		local err, bbox, objs = PlacePrefab(prefab_name, pos, map_id, angle, height_op)
		for _, obj in ipairs(objs) do
			if IsValid(obj) and obj:IsValidPos() and obj:GetVisible() then
				flight_system:Mark(obj)
...
function LevelPrefabController:PlacePrefab()
	...
	local map_id = self.city:GetMapID()
	...
	if not self.objs then
		...
		realm:SuspendPassEdits("PlacePrefab")
		local offset = prefab.dummy_offset or point30
		local prefab_pos = pos - Rotate(offset, angle)
		local ground_offsets = {}
		local cursors = {}
		local err, objs = PlacePrefabObjs(prefab_name, prefab_pos, map_id, 0, ground_offsets)
		for _, obj in ipairs(objs) do
			if IsValid(obj) and obj:IsValidPos() and obj:GetVisible() then
		...
		self.last_pos = pos
		self.objs = cursors
		realm:ResumePassEdits("PlacePrefab")
	end
